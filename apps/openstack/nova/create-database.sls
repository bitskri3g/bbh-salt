...
create_{{ service }}_api_db:
  mysql_database.present:
    - name: {{ service }}_api
    - connection_unix_socket: /var/run/mysqld/mysqld.sock

create_{{ service }}_cell0_db:
  mysql_database.present:
    - name: {{ service }}_cell0
    - connection_unix_socket: /var/run/mysqld/mysqld.sock
...
grant_{{ service }}_api_privs_{{ host }}:
   mysql_grants.present:
    - grant: all privileges
    - database: {{ service }}_api.*
    - user: {{ service }}
    - host: {{ pillar [service + '_configuration']['hosts'][loop.index0] }}
    - connection_unix_socket: /var/run/mysqld/mysqld.sock

grant_{{ service }}_cell0_privs_{{ host }}:
   mysql_grants.present:
    - grant: all privileges
    - database: {{ service }}_cell0.*
    - user: {{ service }}
    - host: {{ pillar [service + '_configuration']['hosts'][loop.index0] }}
    - connection_unix_socket: /var/run/mysqld/mysqld.sock
